define(
  ...
  function(dom, styles, storage, welcome) {
    ...
    Terminal.prototype = (function(dom, styles, storage, welcome) {
       ...
       * Prints the given text to the STDOUT.
       * @params {String} text - The text to be printed.
       * @params {String} link - The link to be printed with <a> tag.
       */
      function printlink(text, link) {
        ...
        line.appendChild(dom.text(text))
        a.appendChild(dom.text(/.*\/(.*)$/g.exec(link)[1]))
      ...
      function show_header() {
        print('Browser Terminal v0.1')
        print('Created by Aeol')
        print('Adapted by mcdulltii')
        print('Built with Vanilla JavaScript')
        printlink('Open-sourced in Github: ', 'https://github.com/mcdulltii/webterm')
        new_line()
      }
      ...
      async function run_intro() {
        ...
        await set_config('prompt-user', 'wwwroot')
