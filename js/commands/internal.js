          ...
          fetch(url)
            ...
            .then(data => {
              ...
              feed = [...feed].filter(function(value) {
                if (value.localName == 'entry') return value
              })
              var items = []
              for (var i = 0;;) {
                ...
                for (var j = 0; j < child.length; j++) {
                  if (child[j].localName == 'title') each.push(child[j].innerHTML)
                  if (child[j].localName == 'link' && child[j].attributes[0].localName == 'href') each.push(child[j].attributes[0].value) 
                }
                if (i > 4) break
                if (each.length == 2) {
                  items.push(each)
